...
for s in plan['segments']:
    ...
    segments.append({'id':s['id'],'seconds_planned':30,'seconds_actual':latest.get('duration'),'generation_status':latest.get('status','not_submitted'),'visual_status':qc.get('status','pending'),'references_ready':not missing,'missing_references':missing,'video':str(v.relative_to(p)) if v and v.exists() else None})
...
for s in segments:
    ...
    rows.append(f'<section><h2>{s["id"]} · 30秒</h2><p>生成：{s["generation_status"]}；画面验收：{s["visual_status"]}；参考：{"就绪" if s["references_ready"] else "待完成前段接点或首帧修正"}</p>{vid}</section>')
